...
{
    internal class DictionaryElementNode : ITypedElement, IAnnotated, IExceptionSource, IShortPathGenerator, IFhirValueProvider, IResourceTypeSupplier
    {
        ...
        internal DictionaryElementNode(ModelInspector inspector, Base root, string rootName = null)
        {
            Current = root;
        ...
        private DictionaryElementNode(ModelInspector inspector, Base instance, DictionaryElementNode parent, PropertyMapping definition, string location, string shortPath)
        {
            Current = instance;
            _inspector = inspector;

            Definition = definition ?? throw Error.ArgumentNull(nameof(definition), "at " + location);
        ...
        public IEnumerable<ITypedElement> Children(string name)
        {
            ...
            DictionaryElementNode getEltNode(object child, PropertyMapping childElementDef, int arrayIndex)
            {
                var location = Location == null
                    ? name
                    : $"{Location}.{name}[{arrayIndex}]";
                var shortPath = ShortPath == null
                    ? name
                    : (childElementDef.IsCollection ?
                        $"{ShortPath}.{name}[{arrayIndex}]" :
                        $"{ShortPath}.{name}");

                return new DictionaryElementNode(_inspector, child as Base, this, childElementDef,
                    location, shortPath);
            }


            var dict = parentBase as IReadOnlyDictionary<string, object>;

            var values = name is { }
                ? dict.TryGetValue(name, out var match)
                    ? new[] { new NamedValue(name, match) }
                    : new NamedValue[] { }
                : dict.Select(kvp => new NamedValue(kvp.Key, kvp.Value));

            foreach (var value in values)
            {
                var childElementDef = _myClassMapping.FindMappedElementByName(value.Name);
                if (childElementDef.IsCollection &&
                    value.Value is IEnumerable en)
                {
                    var i = 0;
                    foreach (var el in en)
                    {
                        yield return getEltNode(el, childElementDef, i++);
                    }
                }
                else
                    yield return getEltNode(value.Value, childElementDef, 0);
            }
        }
